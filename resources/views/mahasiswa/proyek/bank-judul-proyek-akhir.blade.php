...
@if(isset($data) && count($data) > 0)
    <div class="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        @foreach($data as $item)
            <div class="p-6 border-l-4 border-blue-500 rounded-2xl mb-4 bg-white shadow-lg hover:shadow-2xl transition-all duration-300 hover:transform hover:scale-105 bg-gradient-to-br from-white to-gray-50">
                <div class="flex items-center mb-3">
                    ...
                    <h3 class="font-bold text-xl text-blue-700 leading-tight">
                        {{ $item->judul }}
                    </h3>
                </div>
                <p class="text-sm text-gray-500 mb-2 font-medium">
                    Bidang: <span class="text-indigo-600">{{ $item->bidang }}</span>
                </p>
...
@else
    <div class="p-8 bg-gradient-to-r from-yellow-100 to-orange-100 border-l-4 border-yellow-400 rounded-2xl shadow-lg">
        <div class="flex items-center">
            <svg class="w-12 h-12 text-yellow-600 mr-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L3.732 16.5c-.77.833.192 2.5 1.732 2.5z"></path></svg>
            <p class="text-yellow-800 text-lg font-semibold">
                Data bank judul proyek belum tersedia.
            </p>
        </div>
    </div>
@endif
