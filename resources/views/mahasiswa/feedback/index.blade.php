...
<div class="space-y-8">
    ...
    <div class="flex justify-between items-center pb-4 border-b border-gray-200">
        <h2 class="text-3xl font-bold text-gray-800 flex items-center">
            <i class="fas fa-comment-dots mr-3 text-primary-600"></i> Feedback Beasiswa
        </h2>
    </div>
    ...
    {{-- NOTIFIKASI --}}
    @if (session('success'))
        <div class="p-4 mb-4 text-sm text-green-800 rounded-lg bg-green-50 flex items-center" role="alert" x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 5000)">
            <i class="fas fa-check-circle mr-2"></i> {{ session('success') }}
        </div>
    @endif
    @if (session('error'))
        <div class="p-4 mb-4 text-sm text-red-800 rounded-lg bg-red-50 flex items-center" role="alert" x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 5000)">
            <i class="fas fa-times-circle mr-2"></i> {{ session('error') }}
        </div>
    @endif
    @if ($errors->any())
        <div class="p-4 shadow-md bg-red-100 border-l-4 border-red-500 text-red-700 mb-6">
    ...
    {{-- FORM FEEDBACK --}}
    <div class="bg-white shadow-lg rounded-lg overflow-hidden border-l-4 border-primary-500">
        ...
        <div class="p-6 pb-12">
            ...
            @elseif ($sudahMengisi)
                <div class="p-4 mb-4 text-sm text-blue-800 rounded-lg bg-blue-50 border border-blue-200 flex items-center">
            ...
            <form action="{{ route('mahasiswa.feedback.store') }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <textarea 
                        class="w-full p-3 border rounded-lg focus:ring-primary-500 focus:border-primary-500 @error('isi_feedback') border-red-500 @enderror" 
                        id="isi_feedback" 
                        name="isi_feedback" 
                        rows="6" 
    ...
    <div class="bg-white shadow-lg rounded-lg overflow-hidden">
        <div class="p-4 bg-gray-50 border-b border-gray-200">
            <h5 class="text-xl font-semibold text-gray-800 flex items-center">
                <i class="fas fa-history mr-2"></i> Riwayat Feedback Anda (Total: {{ $riwayatFeedback->count() }})
            </h5>
        </div>
        <div class="p-4">
            @if ($riwayatFeedback->isEmpty())
                <div class="p-4 text-center text-blue-800 rounded-lg bg-blue-50 border border-blue-200 flex items-center justify-center">
